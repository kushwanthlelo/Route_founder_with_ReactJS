    ...
    {  res.cookie('chosenDate', req.body.date);
     ...
     converter(journey_time,converted_journey_time)
      ...
      for(let count=0;count<waiting_time.length;count++)
      {
        ...
        details.totalprice=distprice[count][1]
        details.totaldistance=distprice[count][0]
        // console.log(waiting_time,travelling_time,journey_time)
        details.waiting_time=converted_waiting_time[count]
        details.travelling_time=converted_travelling_time[count]
        details.journey_time=converted_journey_time[count]
        paths_="";
        all_intervals.push[[]]
        for(let i=0;i<paths[count].length;i++)
        {
            
            paths_+=paths[count][i];
            
            if(i!=paths[count].length-1)
             {  
                paths_+='-( '
                paths_+=all_intervals[count][i]
                paths_+=" )->"
             }
        }
